# Start uvicorn as a background process and run mock submission test with linking
...
$linkPayload = @{ client_email = "testclient@example.com"; trainer_code = $code } | ConvertTo-Json -Depth 5
Write-Output "Linking client to trainer..."
try {
    $linkRes = Invoke-RestMethod -Uri "$uri/client/link-trainer" -Method Post -Body $linkPayload -ContentType "application/json"
    Write-Output "Link response: $($linkRes | ConvertTo-Json -Depth 5)"
} catch {
    Write-Error "Link failed: $_"
}
